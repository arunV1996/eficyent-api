...
/**
 * Consumer for the "Callback" queue.
 *
...
const processor = async (job: Job): Promise<void> => {
    ...
    console.log(
        `[worker:${QueueNames.Callback}] [stub] job ${job.id} (${job.name})`,
        JSON.stringify(job.data),
    );
};

export const callbackWorker: Worker = createQueueWorker(
    QueueNames.Callback,
    "WORKER_CALLBACK_CONCURRENCY",
    processor,
);
