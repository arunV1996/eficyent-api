...
/**
 * Consumer for the "Deposit" queue.
 *
...
const processor = async (job: Job): Promise<void> => {
    ...
    console.log(
        `[worker:${QueueNames.Deposit}] [stub] job ${job.id} (${job.name})`,
        JSON.stringify(job.data),
    );
};

export const depositWorker: Worker = createQueueWorker(
    QueueNames.Deposit,
    "WORKER_DEPOSIT_CONCURRENCY",
    processor,
);
