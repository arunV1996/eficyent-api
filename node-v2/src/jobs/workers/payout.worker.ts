...
const processor = async (job: Job): Promise<void> => {
    ...
    console.log(
        `[worker:${QueueNames.Payout}] [stub] job ${job.id} (${job.name})`,
        JSON.stringify(job.data),
    );
};

export const payoutWorker: Worker = createQueueWorker(
    QueueNames.Payout,
    "WORKER_PAYOUT_CONCURRENCY",
    processor,
);
