...
/**
 * Consumer for the "CalizaWebhook" queue.
 *
...
const processor = async (job: Job): Promise<void> => {
    ...
    console.log(
        `[worker:${QueueNames.CalizaWebhook}] [stub] job ${job.id} (${job.name})`,
        JSON.stringify(job.data),
    );
};

export const calizaWebhookWorker: Worker = createQueueWorker(
    QueueNames.CalizaWebhook,
    "WORKER_CALIZA_WEBHOOK_CONCURRENCY",
    processor,
);
