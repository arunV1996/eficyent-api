...
/**
 * Consumer for the "BulkPayout" queue.
 *
...
const processor = async (job: Job): Promise<void> => {
    ...
    console.log(
        `[worker:${QueueNames.BulkPayout}] [stub] job ${job.id} (${job.name})`,
        JSON.stringify(job.data),
    );
};

export const bulkPayoutWorker: Worker = createQueueWorker(
    QueueNames.BulkPayout,
    "WORKER_BULK_PAYOUT_CONCURRENCY",
    processor,
);
