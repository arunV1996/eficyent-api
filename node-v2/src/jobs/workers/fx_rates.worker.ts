...
/**
 * Consumer for the "FxRates" queue.
 *
...
const processor = async (job: Job): Promise<void> => {
    ...
    console.log(
        `[worker:${QueueNames.FxRates}] [stub] job ${job.id} (${job.name})`,
        JSON.stringify(job.data),
    );
};

export const fxRatesWorker: Worker = createQueueWorker(
    QueueNames.FxRates,
    "WORKER_FX_RATES_CONCURRENCY",
    processor,
);
