...
export const ledgerController = {
	async index(req: Request, res: Response): Promise<Response> {
		...
		if (q.transaction_type === TRANSACTION_TYPE_CREDIT || q.transaction_type === TRANSACTION_TYPE_DEBIT) {
			let typeNum = q.transaction_type;
			if (q.bank_account_id) {
				typeNum = q.transaction_type === TRANSACTION_TYPE_CREDIT ? TRANSACTION_TYPE_DEBIT : TRANSACTION_TYPE_CREDIT;
			}
			
			const wtRows = await prisma().walletTransaction.findMany({
				where: { type: typeNum, userId: req.user.id },
				select: { id: true },
			});
			const wtIds = wtRows.map((r) => r.id);
			const primaryMorph = q.transaction_type === TRANSACTION_TYPE_CREDIT
				? MORPH_DEPOSIT_TRANSACTION
				: MORPH_BENEFICIARY_TRANSACTION;
			...
			if (wtIds.length > 0) {
				...
				(where.AND as any[]).push({
					OR: [
						...
						{ transactionType: MORPH_WALLET_TRANSACTION, transactionId: { in: wtIds } }
					]
				});
			} else {
		...
		if (req.teamMember && req.teamMember.role === TEAM_MEMBER_ROLE_CORPORATE) {
			...
			const depTransactions = walletId
				? []
				: await prisma().depositTransaction.findMany({
					where: {
						userId: req.user.id,
						teamMemberId: req.teamMember.id,
		...
		if (!req.teamMember && (q.bank_account_id || q.wallet_id)) {
			...
			const ncDepTxns = nonCorpWalletId
				? []
				: await prisma().depositTransaction.findMany({
					where: {
						userId: req.user.id,
			...
			// Wallet-conversion transactions sourced from this VA or associated with this wallet.
			...
			const ncWalletTxns = nonCorpVaId
				? await prisma().walletTransaction.findMany({
					...
					orderBy: { createdAt: "asc" },
					select: { id: true, createdAt: true, type: true, amount: true, quote: { select: { totalSendingAmount: true } } },
				} as any)
				: (nonCorpWalletId
					? await prisma().walletTransaction.findMany({
						where: {
							userId: req.user.id,
							walletId: nonCorpWalletId,
							type: TRANSACTION_TYPE_CREDIT,
						},
						orderBy: { createdAt: "asc" },
						select: { id: true, createdAt: true, type: true, amount: true },
					})
					: []);
			...
			for (const w of ncWalletTxns as any[]) {
				let amt = 0;
				if (nonCorpVaId) {
					amt = -Number(w.quote?.totalSendingAmount ?? 0);
				} else {
					amt = Number(w.amount ?? 0);
				}
	...
	async show(req: Request, res: Response): Promise<Response> {
		...
		if (req.teamMember && req.teamMember.role === TEAM_MEMBER_ROLE_CORPORATE && row) {
			const depAgg = row.walletId
				? { _sum: { totalAmount: null } }
				: await prisma().depositTransaction.aggregate({
					where: {
						userId: req.user.id,
						teamMemberId: req.teamMember.id,
						status: 1, // 1 = COMPLETED
	...
	async export(req: Request, res: Response): Promise<Response> {
		...
		const { TRANSACTION_TYPE_CREDIT, TRANSACTION_TYPE_DEBIT } = await import("../../helpers/constants");
		if (q.transaction_type === TRANSACTION_TYPE_CREDIT || q.transaction_type === TRANSACTION_TYPE_DEBIT) {
			let typeNum = q.transaction_type;
			if (q.bank_account_id) {
				typeNum = q.transaction_type === TRANSACTION_TYPE_CREDIT ? TRANSACTION_TYPE_DEBIT : TRANSACTION_TYPE_CREDIT;
		...
		if (fileType === "excel" || fileType === "xlsx") {
			const exportRows = enriched.map((l: any, i) => {
				...
				const row: Record<string, any> = {
					...
					"Client Ref No": `'${res.client_reference_id || ""}`,
...
async function loadTransaction(
	...
	switch (ledger.transactionType) {
		...
		case MORPH_BENEFICIARY_TRANSACTION: {
			...
			return Object.assign(enrichedLedger, { transaction: t, walletTransaction });
		}
