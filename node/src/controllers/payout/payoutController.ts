...
async function handleSupportingDocument(doc: string | undefined, userId: bigint): Promise<string | undefined> {
...
export const payoutController = {
	...
	async getFormFields(req: Request, res: Response): Promise<Response> {
		...
		const transaction = await transactionFormFields(req.user, q.type, q.country);
	...
	async transactionFormFields(req: Request, res: Response): Promise<Response> {
		const q = req.query as { type?: string; country?: string };
		return sendResponse(res, "", 200, {
			form_fields: await transactionFormFields(req.user, q.type, q.country),
		});
	},
	...
	async retryExternalService(req: Request, res: Response): Promise<Response> {
		...
		if (originalLedger) {
			...
			if (refundLedger) {
				const { computeBankBalance, getWalletBalance } = await import("../../services/virtualAccounts/balanceService");
				...
				if (transaction.teamMemberId) {
					const tm = await prisma().teamMember.findUnique({
						where: { id: transaction.teamMemberId },
					});
					if (tm) {
				...
				const quote = await prisma().quote.findUnique({ where: { id: transaction.quoteId! } });
				if (quote) {
					...
					if (quote.sourceType === MORPH_VIRTUAL_ACCOUNT) {
						...
						if (va) {
							currentBalance = await computeBankBalance(user, va, teamMemberContext);
						}
					} else if (quote.sourceType === MORPH_WALLET) {
						const wallet = await prisma().wallet.findUnique({ where: { id: quote.sourceId! } });
						if (wallet) {
							currentBalance = await getWalletBalance(user, wallet);
						}
					}

					const remitterDepositEnabled = await isRemitterDepositEnabled(user.merchantId);
					if (!remitterDepositEnabled && currentBalance.lt(transaction.totalAmount)) {
						throw new ApiException(154); // Insufficient Balance
					}
				}
		...
		if (sender) {
			...
			if (req.user.merchantId) {
				const merchant = await prisma().merchant.findFirst({
					where: { id: req.user.merchantId as any },
				});
				if (merchant?.name) {
					resolvedBusinessName = merchant.name;
				}
			}
			if (!resolvedBusinessName && Number(req.user.userType) === USER_TYPE_BUSINESS) {
				resolvedBusinessName = userInfo?.businessName ?? "";
			}
			senderName = resolvedBusinessName || `${req.user.firstName ?? ""} ${req.user.lastName ?? ""}`.trim();
		}
	...
	async downloadList(req: Request, res: Response): Promise<Response> {
		...
		const rows = await prisma().beneficiaryTransaction.findMany({
			where,
			orderBy: { createdAt: "desc" },
			include: { beneficiaryAccount: true, quotes: true, senders: true },
		});
		const walletIds: bigint[] = [];
		const vaIds: bigint[] = [];
		for (const r of rows) {
			if (r.quotes) {
				const sourceId = r.quotes.sourceId;
				if (sourceId) {
					if (r.quotes.sourceType === "App\\Models\\Wallet" || r.quotes.sourceType === "wallet") {
						walletIds.push(BigInt(sourceId));
					} else if (r.quotes.sourceType === "App\\Models\\VirtualAccount" || r.quotes.sourceType === "virtual_account") {
						vaIds.push(BigInt(sourceId));
					}
				}
				if (r.quotes.virtualAccountId) {
					vaIds.push(BigInt(r.quotes.virtualAccountId));
				}
			}
		}

		const [wallets, virtualAccounts] = await Promise.all([
			walletIds.length > 0 ? prisma().wallet.findMany({ where: { id: { in: walletIds } } }) : [],
			vaIds.length > 0 ? prisma().virtualAccount.findMany({ where: { id: { in: vaIds } } }) : [],
		]);
		const walletMap = new Map(wallets.map((w) => [w.id.toString(), w.currency]));
		const vaMap = new Map(virtualAccounts.map((v) => [v.id.toString(), v.currency]));
		...
		const exportRows = rows.map((r, i) => {
			...
			if (r.quotes) {
				...
				if (sourceId) {
					if (r.quotes.sourceType === "App\\Models\\Wallet" || r.quotes.sourceType === "wallet") {
						sendingCurrency = walletMap.get(sourceId.toString()) ?? "";
					} else if (r.quotes.sourceType === "App\\Models\\VirtualAccount" || r.quotes.sourceType === "virtual_account") {
						sendingCurrency = vaMap.get(sourceId.toString()) ?? "";
					}
				}
				if (!sendingCurrency && r.quotes.virtualAccountId) {
					sendingCurrency = vaMap.get(r.quotes.virtualAccountId.toString()) ?? "";
				}
			}

			let remitterName = "";
			if (r.senders) {
				if (Number(r.senders.type) === 2) {
					remitterName = r.senders.firstName ?? "";
				} else {
					remitterName = [r.senders.firstName, r.senders.lastName].filter(Boolean).join(" ");
				}
			} else {
				if (merchant) {
					remitterName = merchant.name;
				} else {
					remitterName = [req.user?.firstName, req.user?.lastName].filter(Boolean).join(" ");
				}
			}

			let beneficiaryName = "";
			if (r.beneficiaryAccount) {
				if (Number(r.beneficiaryAccount.type) === 2) {
					beneficiaryName = r.beneficiaryAccount.businessName || [r.beneficiaryAccount.firstName, r.beneficiaryAccount.lastName].filter(Boolean).join(" ");
				} else {
					beneficiaryName = [r.beneficiaryAccount.firstName, r.beneficiaryAccount.lastName].filter(Boolean).join(" ");
				}
			}

			const statusText = beneficiaryTransactionStatusLabel(Number(r.status));

			const quote = r.quotes || (r as any).quote;

			let fxRateFormatted = "";
			if (quote?.fxRate && sendingCurrency && r.receivingCurrency) {
				fxRateFormatted = `1${sendingCurrency} = ${quote.fxRate}${r.receivingCurrency}`;
			} else {
				fxRateFormatted = quote?.fxRate ?? "";
			}

			return {
				"S. No.": i + 1,
				"Transaction ID": r.txnRefNo ?? "",
				"Client Ref No": r.clientReferenceId ?? "",
				"Sending Amount": r.totalAmount.toString(),
				"Sending Currency": sendingCurrency,
				"Recipient Amount": r.recipientAmount?.toString() ?? "",
				"Recipient Currency": r.receivingCurrency ?? "",
				"Fees": r.commissionAmount.toString(),
				"FX Rate": fxRateFormatted,
				"Remitter Name": remitterName,
				"Beneficiary Name": beneficiaryName,
				"Account Number": r.beneficiaryAccount?.accountNumber ?? "",
				"Remarks": r.remarks ?? "",
	...
	async payoutTemplate(req: Request, res: Response): Promise<Response> {
		...
		const flat = flattenFormFields(
			...
			["quote", "beneficiary", ...(req.user.enableSender ? ["remitter"] : [])],
		);
	...
	async bulkStore(req: Request, res: Response): Promise<Response> {
		...
		const { MORPH_WALLET, MORPH_VIRTUAL_ACCOUNT } = await import("../../helpers/constants");
		const { getVirtualAccountScope } = await import("../../services/virtualAccounts/virtualAccountService");

		const bankAccountId = (req.body as { bank_account_id?: string }).bank_account_id;
		const walletId = (req.body as { wallet_id?: string }).wallet_id;
		...
		if (walletId) {
			const wallet = await prisma().wallet.findFirst({
				where: { uniqueId: walletId, userId: req.user.id },
			});
			if (wallet) {
				sourceType = MORPH_WALLET;
				sourceId = wallet.id.toString();
			}
		} else if (bankAccountId) {
			const vaScope = await getVirtualAccountScope(req.user);
			const va = await prisma().virtualAccount.findFirst({
				where: { ...vaScope, uniqueId: bankAccountId },
			});
			if (va) {
				sourceType = MORPH_VIRTUAL_ACCOUNT;
				sourceId = va.id.toString();
			}
		}
		...
		const fields = flattenFormFields(
			{ quote, beneficiary, remitter },
			["quote", "beneficiary", ...(enableSender ? ["remitter"] : [])],
		);
		const {
		...
		const result = await processExcel(buffer, fields, async (payload, rowNumber) => {
			...
			let sen = null;
			if (enableSender) {
				sen = await validateAndNormalizeSender(
