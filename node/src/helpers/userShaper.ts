...
export function genderFormatted(gender: string | null | undefined): string {
	...
	if (g === "male" || g === "1" || g === "m") return "Male";
	if (g === "female" || g === "2" || g === "f") return "Female";
	if (g === "others" || g === "3" || g === "o") return "Others";
	return gender;
}
...
export async function shapeUserInfo(
	...
	if (Number(user.userType) === USER_TYPE_BUSINESS) {
		const typeOfBusinessFormatted = info?.type_of_business
			? await lookupsService.findValuebyKey(info.type_of_business, "business_types")
			: "";
		...
		const professionFormatted = info?.profession
			? await lookupsService.findValuebyKey(info.profession, "professions")
			: "";
		const sourceOfIncomeFormatted = info?.sourceOfIncome
			? await lookupsService.findValuebyKey(info.sourceOfIncome, "sources_of_incomes")
			: "";
		const idTypeFormatted = info?.idType
			? await lookupsService.findValuebyKey(info.idType, "id_types")
			: "";
		...
		return {
			user_information: {
				...
				postal_code: info?.postalCode ?? "",
				purpose_of_transactions: info?.purposeOfTransactions ?? "",
				id_type: idTypeFormatted || (info?.idType ?? ""),
