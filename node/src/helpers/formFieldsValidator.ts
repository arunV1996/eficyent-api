...
function validateScalar(
	...
	if (!isPresent) {
		if (isMandatory) pushErr(errors, fullKey, `${field.field_label} is required.`);
		return value;
	}
	...
	if (field.values_supported && field.values_supported.length > 0) {
		const lower = field.values_supported.map((v2) => String(v2.value).trim().toLowerCase());
...
function validateField(
	...
	if (!isMandatory && field.required_if_empty_of && source) {
		const otherVal = source[field.required_if_empty_of];
		const otherIsPresent =
			otherVal !== undefined &&
			otherVal !== null &&
			!(typeof otherVal === "string" && otherVal.length === 0);
		if (!otherIsPresent) {
			isMandatory = true;
		}
	}
	if (!isMandatory && field.required_if && source) {
		const otherVal = source[field.required_if];
		const otherIsPresent =
			otherVal !== undefined &&
			otherVal !== null &&
			!(typeof otherVal === "string" && otherVal.length === 0);
	...
	if (field.field_type === "group") {
		if (field.is_repeatable) {
			if (!Array.isArray(value)) {
				if (isMandatory) pushErr(errors, fullKey, `${field.field_label} must be an array.`);
				return [];
			}
