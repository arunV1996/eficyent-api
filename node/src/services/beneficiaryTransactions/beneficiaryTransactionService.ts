...
export async function createPayoutTransaction(
	...
	if (payload.remitter_id) {
		...
		if (
			(beneficiaryAccount.currency === "PKR" && sender.nationality === "IND") ||
			(beneficiaryAccount.currency === "INR" && sender.nationality === "PAK")
		) {
	...
	const created = await prisma().$transaction(async (tx) => {
		...
		const txn = await tx.beneficiaryTransaction.create({
			data: {
				...
				orderId: payload.order_id ?? `TXN${Math.floor(Date.now() / 1000).toString().slice(-8)}${Math.random().toString(36).substring(2, 6).toUpperCase()}`,
				status: finalStatus,
			},
		});
	...
	for (const uid of uniqueIds) {
		try {
			const updated = await prisma().$transaction(async (tx) => {
				...
				if (!allowedForCancellation.includes(txn.status)) {
