...
<body class="statement">
        ...
        <tr>
            <td valign="bottom">
                <p style="font-size: 12px;font-weight: 300;color: #777777;">
                    <strong style="color: #111111;">{{tr('deposits')}} </strong> | {{ now()->format('m/d/Y') }}
                </p>
            </td>
            <td align="right">
    ...
    <table class="statement-table" style="margin-top: 20px;">
        ...
        <tbody>
            @foreach($deposit_details as $i => $export_detail)

            <tr>

                <td>{{ $i+1 }}</td>

                <td>{{ $export_detail['unique_id'] }}</td>

                <td>{{ $export_detail['memo'] }}</td>

                <td>{{ $export_detail['amount'] }} {{ $export_detail['currency'] }}</td>

                <td>{{ deposit_transaction_status_formatted($export_detail['status']) }}</td>
